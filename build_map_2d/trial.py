...
class HectorMapper:
    def __init__(self):
        ...
        self.prev_dtheta = 0.0  # Góc thay đổi trước đó
        self.prev_time = None  # Thời gian scan trước đó
        ...
        # Tham số ICP
        self.icp_max_iterations = 100  # Tăng số lượng iterations
        self.icp_threshold = 0.0001  # Ngưỡng hội tụ
        self.icp_min_points = 50  # Số lượng điểm tối thiểu
        self.icp_distance_threshold = 0.5  # Ngưỡng khoảng cách cho điểm tương ứng

        # Giới hạn chuyển động
        self.max_translation = 1.0  # mét
        self.max_angular_change = 1.0  # radian (giới hạn thay đổi góc tối đa giữa các lần quét)
        self.max_angular_velocity = 1.5  # radian/s (giới hạn tốc độ góc)
        self.last_scan_time = rospy.Time.now()

        # Lưu trữ lịch sử vận tốc góc để nội suy
        self.angular_velocity_history = []
        self.max_history_size = 500  # Số lượng vận tốc góc lưu trữ để nội suy
    ...
    def icp_scan_matching(self, prev_points, current_points, dt):
        ...
        Nội suy vận tốc góc từ các khung scan khác nhau.
        ...
        # Biến để kiểm tra trường hợp quay tại chỗ
        is_rotating = False
        ...
        for iteration in range(self.icp_max_iterations):
            ...
            # Kiểm tra trường hợp quay tại chỗ (chuyển động tịnh tiến nhỏ)
            translation = np.sqrt(dx**2 + dy**2)
            if translation < 0.05:  # Ngưỡng nhỏ để phát hiện quay tại chỗ
                is_rotating = True
        ...
        # 4. Nội suy vận tốc góc
        if dt > 0:
            angular_velocity = best_transform[2] / dt  # radian/s
            ...
            if abs(angular_velocity) > self.max_angular_velocity:
                ...
                best_transform[2] = angular_velocity * dt
                rospy.logwarn("Giới hạn vận tốc góc")

            # Lưu vận tốc góc vào lịch sử
            ...
            # Nội suy vận tốc góc bằng trung bình của lịch sử
            if self.angular_velocity_history:
                estimated_angular_velocity = np.mean(self.angular_velocity_history)
            else:
                estimated_angular_velocity = angular_velocity

        # 5. Hiệu chỉnh dtheta nếu đang quay tại chỗ
        if is_rotating:
            # Sử dụng vận tốc góc nội suy để tính dtheta
            dtheta_corrected = estimated_angular_velocity * dt
            best_transform[2] = dtheta_corrected
            rospy.loginfo(f"Quay tại chỗ, hiệu chỉnh dtheta: {dtheta_corrected:.3f}, angular_velocity: {estimated_angular_velocity:.3f}")
